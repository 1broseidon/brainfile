...
import { getEffectiveDestination, getArchiveConfig } from '../utils/config';
import {
  isV2,
  getV2Dirs,
  buildBoardFromV2,
...
/**
 * Resolve the on-disk `.md` file backing a document, board or archive.
 *
 * This is what the `e` $EDITOR handoff opens (§C1): the real per-task file,
 * not a synthetic YAML snapshot that has to be diffed back into a patch. v1
 * boards have no per-task file — every task lives inside the single board
 * document — so they get an explicit refusal rather than a confusing failure.
 */
export function resolveTaskFilePath(
  ...
  if (!isV2(filePath)) {
    return {
      success: false,
      error: 'Editing a document file needs a v2 board. Run: brainfile migrate',
    };
  }

  try {
    const found = findV2Task(getV2Dirs(filePath), taskId, true);
    if (!found) return { success: false, error: `Document ${taskId} not found` };
