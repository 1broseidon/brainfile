...
try {
  const res = await fetch('https://api.github.com/repos/1broseidon/brainfile', {
    headers: {
  ...
  await writeFile(OUT, `${JSON.stringify({ ...current, brainfile: repo.stargazers_count }, null, 2)}\n`)
  console.log(`stars: refreshed brainfile=${repo.stargazers_count}`)
} catch (err) {
